...
def _pi_workflow_steps(pi):
    """Describe independent PI workflow records and their real blocking rules."""
    def step(key, label, icon, state, status, reason='', reason_label='下一步'):
        if state == 'blocked':
            reason_label = '阻塞原因'
        return {
            'key': key,
    ...
    active_payments = list(pi.active_payments)
    has_payment = (pi.received_amount or 0) > 0
    paid_with_fees = sum(
        float(payment.amount or 0) + float(payment.fee or 0)
        for payment in active_payments
    )
    currency_symbol = '¥' if (pi.currency or 'USD').upper() == 'RMB' else '$'
    remaining = max(float(pi.grand_total or 0) - paid_with_fees, 0)

    if pi.paid:
        payment_step = step(
            'payment', '回款', 'bi-cash-coin', 'complete', '已付清',
            f'已登记 {len(active_payments)} 笔有效回款', '记录',
        )
    elif has_payment:
        payment_reason = (
            f'距付清还差 {currency_symbol}{remaining:,.2f}'
            if remaining > 0 else '有效回款已达到订单金额，付款状态尚未更新'
        )
        payment_step = step(
            'payment', '回款', 'bi-cash-coin', 'active', '部分回款',
            payment_reason,
        )
    else:
        payment_step = step(
            'payment', '回款', 'bi-cash-coin', 'pending', '未回款',
            '等待登记首笔回款',
        )
    ...
    missing_purchase_items = [
        item for item in pi.items
        if purchased_by_item.get(item.id, 0) < int(item.quantity or 0)
    ]
    purchase_quantities_complete = bool(pi.items) and not missing_purchase_items
    if pi.procurement_is_complete:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'complete', '采购完成',
            f'已确认 {len(pi.procurements)} 条采购明细', '记录',
        )
    elif pi.procurement_confirmed and missing_purchase_items:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'blocked', '数据待核对',
            f'已标记采购完成，但仍有 {len(missing_purchase_items)} 项数量不足',
        )
    elif pi.procurements and purchase_quantities_complete:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'active', '待确认',
            '采购数量已齐，等待管理员点击“采购完成”',
        )
    elif pi.procurements:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'active', '部分采购',
            f'还有 {len(missing_purchase_items)} 项产品尚未采购足量',
        )
    elif not has_payment:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'blocked', '尚未开始',
            '尚未回款，不能开始采购',
        )
    else:
        procurement_step = step(
            'procurement', '采购', 'bi-cart-check', 'pending', '待采购',
            '已收到回款，等待管理员开始采购',
        )

    packing_list = pi.packing_list
    if packing_list and packing_list.is_completed:
        packing_step = step(
            'packing', '装箱', 'bi-box-seam', 'complete', '装箱完成',
            f'装箱单已完成，共 {len(packing_list.boxes)} 箱', '记录',
        )
    elif packing_list and not has_payment:
        packing_step = step(
            'packing', '装箱', 'bi-box-seam', 'blocked', '草稿已锁定',
            '当前没有有效回款，这份历史草稿只能查看或导出',
        )
    elif packing_list:
        packing_step = step(
            'packing', '装箱', 'bi-box-seam', 'active', '装箱草稿',
            '装箱单尚未保存为已完成',
        )
    elif not has_payment:
        packing_step = step(
            'packing', '装箱', 'bi-box-seam', 'blocked', '尚未创建',
            '尚未回款，不能创建装箱单',
        )
    else:
        packing_step = step(
            'packing', '装箱', 'bi-box-seam', 'pending', '待装箱',
            '已收到回款，等待管理员创建装箱单',
        )

    if pi.customs_required is True:
        customs_step = step(
            'customs', '报关', 'bi-file-earmark-check', 'complete', '需要报关',
            '报关要求已登记', '记录',
        )
    elif pi.customs_required is False:
        customs_step = step(
            'customs', '报关', 'bi-file-earmark-x', 'complete', '无需报关',
            '无需报关的决定已登记', '记录',
        )
    else:
        customs_step = step(
            'customs', '报关', 'bi-file-earmark-question', 'pending', '未登记',
            '尚未登记是否需要报关',
        )

    if pi.shipping_completed:
        shipping_record = pi.shipping_date.isoformat() if pi.shipping_date else '已登记发货'
        if pi.shipping_tracking_no:
            shipping_record += f' · {pi.shipping_tracking_no}'
        shipping_step = step(
            'shipping', '发货', 'bi-truck', 'complete', '发货完成',
            shipping_record, '记录',
        )
    elif not has_payment:
        shipping_step = step(
            'shipping', '发货', 'bi-truck', 'blocked', '不可登记',
            '尚未回款，不能登记发货',
        )
        ...
        shipping_step = step(
            'shipping', '发货', 'bi-truck', 'pending', '待发货',
            '登记条件已满足，等待业务员登记发货',
        )
...
def pi_detail(id):
    ...
    require_pi_access(pi)
    workflow_steps = _pi_workflow_steps(pi)
    resp = make_response(render_template(
        'pi_detail.html',
        pi=pi,
        workflow_steps=workflow_steps,
        workflow_completed_count=sum(
            step['state'] == 'complete' for step in workflow_steps
        ),
        workflow_blocked_count=sum(
            step['state'] == 'blocked' for step in workflow_steps
        ),
    ))
    resp.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    resp.headers['Pragma'] = 'no-cache'
