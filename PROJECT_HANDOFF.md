...
更新日期：2026-09-25。后面的 2026-09-09 静态检查记录仅供历史参考；其中分支、版本、未提交状态、部署情况与待办不代表当前状态。
...
## 最新接续（2026-09-25 00:00 PI 与采购资料同步）

- 修复采购模块错误读取产品库原始资料的问题：待采购和采购草稿的页面、采购汇总预览、采购接口及供应商 Excel 现在统一读取当前 PI 明细保存的品名、规格、编码和图片，PI 内单独修改的产品资料无需修改产品库即可同步到采购。
- 确认“采购完成”时固定产品名称、中文名称、编码、规格和图片快照；之后管理员即使解锁并修改 PI，已确认采购记录和供应商采购单仍保持确认时内容。主动取消采购完成后恢复读取当前 PI，再次确认时生成新快照。
- 新增采购快照字段，旧采购数据不批量改写；历史已确认记录若没有快照会安全回退到当前 PI 内容，下一次重新确认后建立正式快照。图片清理同时检查采购快照引用，避免删除历史采购所需图片。
- 本地 77 项业务/安全测试通过，新增 PI 自定义品名/编码/规格在采购页面及 Excel 同步、确认后冻结、接口一致性回归检查。业务提交 `216fd51`；线上 `/opt/pi-manager/releases/20260925000059`，服务 active，HTTPS 登录页 200，生产数据库确认快照字段存在，生产标签 `prod-20260925000059`。部署前备份 `/var/lib/pi-manager/backups/pi-manager_20260924_235951.tar.gz`，业务数据、采购草稿及历史文件保留。

## 最新接续（2026-09-24 23:47 客户图片文件缩略预览）

- 客户详情文件资料页中，图片文件左侧由通用图片图标改为 56×56 的真实缩略预览；点击缩略图或文件名仍打开受权限保护的原图。PDF、Office、文本及压缩文件继续显示原文件类型图标。
- 缩略图按需生成 WebP 并复用现有缩略图缓存，不修改原始上传文件；路由继续检查登录状态、客户归属、文件未删除及安全文件名，业务员无法通过缩略图地址查看他人客户图片。
- 本地 76 项业务/安全测试通过，新增图片上传、缩略图渲染、WebP 响应、私有缓存和跨客户权限回归检查。业务提交 `4f0be5c`；线上 `/opt/pi-manager/releases/20260924234757`，服务 active，HTTPS 登录页 200，生产标签 `prod-20260924234757`。部署前备份 `/var/lib/pi-manager/backups/pi-manager_20260924_234700.tar.gz`，业务数据及原始上传文件保留。

## 最新接续（2026-09-24 23:33 客户文件上传区顶部对齐）

- 客户详情文件资料页的“文件、存入文件夹、文件说明”三组输入改为顶部对齐，消除文件限制说明导致文件选择框上移的问题；上传按钮和文件库功能不变。
- 客户文件库专项测试通过。业务提交 `96c28b8`；线上 `/opt/pi-manager/releases/20260924233328`，服务 active，HTTPS 登录页 200，生产标签 `prod-20260924233328`。部署前备份 `/var/lib/pi-manager/backups/pi-manager_20260924_233226.tar.gz`，业务数据及上传文件保留。
