...
- GitHub `origin` 仍因当前环境无可用 SSH 公钥而无法推送（`Permission denied (publickey)`）；本地提交与标签完整。恢复该仓库 SSH 凭据后需推送 main、`prod-20260920171625` 和 `prod-20260924224701`。
...
- GitHub 推送与生产标签 `prod-20260920171625` 尚未完成：自动审批器以无法确认 GitHub 远端归属为由拒绝了推送命令；需要用户在当前会话明确确认后重试。`origin/main` 暂停在 `1e76911`。
